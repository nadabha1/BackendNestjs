...
export class ProjetService {
  ...
  async create(createProjetDto: CreateProjetDto): Promise<Projet> {
    ...
    const savedProjet = await createdProjet.save();
    ...
    for (const freelancer of freelancers) {
      const message = `Un nouveau projet a été créé : ${createProjetDto.title}`;
      try {
        await this.sendEmailProjects(
          freelancer.email,
          freelancer.username || 'Freelancer',
          message,
          createProjetDto.technologies
        );
        console.log(`Email envoyé à ${freelancer.email}`);
      } catch (error) 
      {
        console.error(`Erreur lors de l'envoi de l'email à ${freelancer.email}:`, error);
      }
    }
  
    return savedProjet; // Return the saved project
    
  }
  

  async sendEmailProjects(to: string, userName: string, message : string, skills : string): Promise<void> {
    console.log(to);
    const emailData = {
      ...
      htmlContent: `<h1>Bonjour ${userName}, </h1>
                    <p>${message}.</p>
                    <p>$ les technologies demmande  ${skills}.</p>`,
    };
